...
const Spin = ({
  ...
  return /*#__PURE__*/React.createElement("div", null, _tip);
};
...
const InternalMRouterContextProvider = ({
  permissionList: _permissionList = DEFAULT_PERMISSION_LIST,
  routerConfig,
  hasAuth,
  children
}, ref) => {
  ...
  }, [basename, beforeEachMount, hasAuth, location, _permissionList]);
  ...
  useLayoutEffect(() => {
    ...
    if (state.permissionList === _permissionList && hasAuth === state.hasAuth && state.inputRoutes === inputRoutes && state.beforeEachMount === beforeEachMount) {
      return;
    ...
    if (inputRoutesRef.current === inputRoutes && state.permissionList === _permissionList && hasAuth === state.hasAuth && state.beforeEachMount === beforeEachMount) {
  ...
  }, [state.inputRoutes, inputRoutes, _permissionList, state.permissionList, hasAuth, state.hasAuth, basename, beforeEachMount, location]); // auto setting document.title
...
const MRouter = props => {
  return /*#__PURE__*/React.createElement(CoreRouter, Object.assign({}, props, {
