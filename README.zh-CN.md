...
- 🛠 [**[config router 入口配置项](#defineRouterConfig)**] - 集中配置路由，快捷、方便管理。
- ＋ [**[动态添加路由：addRoutes](#useAddRoutes)**] - 动态增加路由：可使用 hook`useAddRoutes`添加路由，自动刷新视图。
- ➖ [**[动态删除路由：removeRoutes](#useRemoveRoutes)**] - 动态删除路由：可使用 hook`useRemoveRoutes`删除路由，自动刷新视图。
- 🖇 [**[动态更新路由：updateRoutes](#useUpdateRoutes)**] - 动态修改路由：可使用 hook`useUpdateRoutes`修改路由。
- 🔐 [**[权限路由配置 permission](#权限路由)**] - 权限控制：配置路由的 code，自动管理路由的权限
- 👨‍✈️‍ [**[路由守卫](#路由守卫)**] - 提供路由进入时的回调 `beforeEnter` and `beforeEachMount`, 路由离开时的钩子 `useBeforeLeave`
- 🌲 [**[路由导航]**] - 层级导航：支持层级导航，自动实现父子级路由的导航栏生成，例如面包屑、菜单导航
...
用法`defineRouterConfig(routerConfig)`
